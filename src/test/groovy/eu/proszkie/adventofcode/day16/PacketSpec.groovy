package eu.proszkie.adventofcode.day16

import spock.lang.Specification

import static OperatorPacketBuilder.operatorPacket
import static eu.proszkie.adventofcode.day16.LiteralValueBuilder.literalValue
import static eu.proszkie.adventofcode.day16.LiteralValuePacketBuilder.literalValuePacket

class PacketSpec extends Specification {

    def 'should add all version numbers in packet and subpackets'() {
        given:
            def aPacket = operatorPacket {
                version 4
                typeId 5
                lengthTypeId 0
                subPackets([
                        literalValuePacket {
                            version 16
                            literalValues([
                                    literalValue { binary '1011' },
                                    literalValue { binary '1011' },
                                    literalValue {
                                        isLast true
                                        binary '0111'
                                    }
                            ])
                        }
                ])
            }

        expect:
            aPacket.sumUpAllVersions() == 20
    }

    def 'should add all version numbers in packet and subpackets of encoded transmission '() {
        given:
            def aPacket = PacketTransmissionFactory.INSTANCE.fromHex(hex)
        expect:
            aPacket.sumUpAllVersions() == expected
        where:
            hex || expected
            '8A004A801A8002F478' || 16
            '620080001611562C8802118E34' || 12
            'C0015000016115A2E0802F182340' || 23
            'A0016C880162017C3686B18A3D4780' || 31
            '' || 981

    }
}
